...
def main(dfData, to_excel):
    ...
    df = pd.DataFrame(columns=("对应评论序号", "对应评论句子", "包含产品特征词", "对应顾客需求", "句子长度", "对应评论星级")) # , "情感预测", "情感计算值"
    for index, row in dfData.iterrows():
        ...
        content = str(dfData.loc[index, '评论内容'])

        # xuhao = dfData.loc[index, '序号']
...
if __name__ == '__main__':
    ...
    # path = '数据/16 顾客需求分类3.xlsx'
    # main(dfData, to_excel=path)

    dfData = pd.read_excel('数据/6 完整数据集（句子产品特征词性统计后）(新2）2.xlsx',index_col=0)
    path = '数据/16 顾客需求分类3.2.xlsx'
    main(dfData, to_excel=path)