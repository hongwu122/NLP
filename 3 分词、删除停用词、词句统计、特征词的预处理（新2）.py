...
def main(dfData, df_Feature, otherProductList, to_excel):
    ...
    for index, row in dfData.iterrows():
        ...
        # 遍历一次，如果特征词不在分词里面，又在评论里面，还是要记下这个特征词。不然就是漏网之鱼
        for d in range(len(df_Feature['顾客需求'])):
            customer_demand = df_Feature['顾客需求'][d]
            # print(customer_demand)
            feature_word = str(df_Feature['特征词'][d]).split(' ')
            ...
            for feature_sole_word in feature_word:
                if (feature_sole_word in rowData) and (feature_sole_word not in product_word) and feature_sole_word != '':
                    # print(rowData)
                    # print(product_word)
                    # print('漏网之鱼 #######',feature_sole_word)
                    product_word_Add = product_word_Add + ' ' + str(feature_sole_word)
                    customer_demand_Add = customer_demand_Add + ' ' + str(customer_demand)
                    productNum = productNum + 1  # 特征词数


        product_word_Add_list = product_word_Add.split(' ')
        if '' in product_word_Add_list:
            product_word_Add_list.remove('')
        # for sentence in sentence_statu:
        # print('当前产品特征词列表：',product_word_Add_list)
        # print(sentence_statu)
        p_count = 0
        for p in product_word_Add_list:
            # print('当前产品特征词：',p)
            # print('当前评论：',rowData)
            # print('分割后的评论：',sentence_statu)
            # print('第几句：',p_count)
            # print('这句评论内容：',sentence_statu[p_count])

            try:  # 偶尔会出现特征词被分句分割的情况，导致找不到分句，pass（解决办法：前后两句一起算）
                while p not in sentence_statu[p_count]:
                    # print('循环中')
                    p_count += 1
                    # print(sentence_statu[p_count])
            except:
                p_count = 0
                # print(len(p))
                for s in range(len(p)):
                    # try:
                    while p[s] not in sentence_statu[p_count]:
                        p_count += 1
                    # except:
                    #     pass
                    try:
                        print('有点特别:', '\n', p[s], '\n', sentence_statu[p_count], p[s + 1], '\n', sentence_statu[p_count + 1])
                        if p[s] in sentence_statu[p_count] and p[s + 1] in sentence_statu[p_count + 1]: # 匹配单个字符
                            sentence_list.append(sentence_statu[p_count] + sentence_statu[p_count + 1]) # 前后两句一起算
                            print('成功了')
                            break
                    except:
                        sentence_list.append(sentence_statu[p_count])
                        break

                # print(p_count)
            if p in sentence_statu[p_count]:
        ...
        dataAdd = pd.Series(
            { "字数": words_num,"形容词数": adj_num, "动词数": verb_num,
              ...
              "提到顾客需求数": len(set(sentence_list)), "提到产品特征的句子数": productSentenceNum, '提到产品特征的句子': sentence_list,
...
if __name__ == '__main__':
    ...
    main(dfData2, df_Feature, otherProductList, to_excel="数据/6 完整数据集（句子产品特征词性统计后）(新2）2.xlsx")
