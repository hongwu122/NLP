...
import pandas as pd
import jieba
# from sklearn.externals import joblib# 老版本
import joblib
...
if __name__ == '__main__':
    import_file = '11 完整数据集（BERT）.xlsx' # 测试数据集2.xlsx    data3 BERT.xlsx   汇总 20000+ 去重 测试.xlsx
    output_file = '12 完整数据集（BERT+SVM）.xlsx' # data3 SVM.xlsx


    ## 获取数据
    dfData = pd.read_excel(import_file)
    dfAdd = pd.DataFrame(columns = ("SVM情感取向","星级有效性分析"))
    # print(dict(zip(dfData['评论内容'],dfData['星级'])).items())
    for rowData,rowData2 in dict(zip(dfData['评论内容'],dfData['星级'])).items():
        string = rowData
        xingji = rowData2
        qinggan = svm_predict(string)
        if xingji <=3 and qinggan =='消极':
            ...
            fenxi = '无效'
        dataAdd = pd.Series({'SVM情感取向':qinggan,'星级有效性分析':fenxi})
