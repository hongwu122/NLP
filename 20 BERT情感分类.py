...
if __name__ == '__main__':
    import_file = '7 完整数据集（情感词典预测）.xlsx' # 测试数据集2.xlsx
    output_file = '11 完整数据集（BERT）.xlsx' # data3 BERT.xlsx
    ...
    # predict(test)
    dfAdd = pd.DataFrame(columns=['BERT情感分类'])
    label_list = predict(df['评论内容'])
    for i in label_list:
        dataAdd = pd.Series({'BERT情感分类': i})
        dfAdd = dfAdd.append(dataAdd, ignore_index=True)

    dfResult = pd.concat([df, dfAdd], axis=1)
